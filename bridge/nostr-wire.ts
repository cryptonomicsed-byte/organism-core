...
export const KIND_ATTESTATION = 47002;
...
export const CRUCIBLE_RESERVED: readonly [number, number] = [47000, 48000];

/** Kinds any organ using this module may publish under. */
const PUBLISHABLE = new Set<number>([KIND_AGENT_ENGRAM, KIND_CLAIM, KIND_AUTH]);
 ...
 */
export function isPublishable(kind: number): boolean {
  return PUBLISHABLE.has(kind);
}

/** True when `kind` falls inside Crucible's reserved block. */
export function isCrucible(kind: number): boolean {
  return kind >= CRUCIBLE_RESERVED[0] && kind < CRUCIBLE_RESERVED[1];
}
 ...
 */
export function canonicalSerialize(e: EventFields): string {
  return JSON.stringify([0, e.pubkey, e.created_at, e.kind, e.tags, e.content]);
}

/** NIP-01 event id: sha256 of the canonical serialization, lowercase hex. */
export function eventId(e: EventFields): string {
  return createHash('sha256').update(canonicalSerialize(e), 'utf8').digest('hex');
}
...
export function validateSlug(slug: string): boolean {
  if (utf8Length(slug) > 255) return false;
 ...
 */
export function buildSlug(namespace: string, ...segments: string[]): string {
 ...
 */
export function buildUnsignedEvent(opts: {
  pubkey: string;
  kind: number;
  content: string;
  tags?: Tag[];
